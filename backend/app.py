...
def api_predict():
    ...
    # Map raw feature names to readable labels (keys for frontend dictionary)
    FEATURE_KEY_MAP = {
    ...
    clean_features = {FEATURE_KEY_MAP.get(k, k): v for k, v in zip(FEATURE_NAMES, feats)}
    
    clean_shap_values = {}
    if SHAP_AVAILABLE and explainer:
            try:
                shap_values = explainer.shap_values(X)[1][0]
                # Map shap values to clean keys
                clean_shap_values = {FEATURE_KEY_MAP.get(k, k): v for k, v in zip(FEATURE_NAMES, shap_values)}
            except Exception:
                pass
